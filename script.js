...
vct.addEventListener('click', () => {
  ...
  setInterval(() => {
    ...
    seventeenBox.style.backgroundColor = 'blue'
  ...
  function spawnBlock() {
    const eighteenBox = document.createElement('div')
    eighteenBox.style.width = '100px'
    eighteenBox.style.height = '100px'
    eighteenBox.style.backgroundColor = 'red'
    eighteenBox.style.position = 'absolute'
    eighteenBox.style.left = `${eighteenArea.clientWidth}px`

    const y = Math.random() * (eighteenArea.clientHeight - 100)

    eighteenBox.style.top = `${y}px`
    
    const eighteenAnimation = (eighteenArea.clientWidth + 100) / 300

    eighteenBox.style.animation = `moveLeft ${eighteenAnimation}s linear forwards`

    eighteenBox.addEventListener('animationend', () => {
      eighteenArea.removeChild(eighteenBox)
    })
